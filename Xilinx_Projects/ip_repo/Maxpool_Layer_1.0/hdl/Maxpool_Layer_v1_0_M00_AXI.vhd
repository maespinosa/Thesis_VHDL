...
entity Maxpool_Layer_v1_0_M00_AXI is
	generic (
	...
	port (
       ...
       i_slv_reg_wren			: in std_logic_vector(C_M_AXI_DATA_WIDTH-1 downto 0); 
...
end Maxpool_Layer_v1_0_M00_AXI;

architecture implementation of Maxpool_Layer_v1_0_M00_AXI is

                                                            

    type master_state is (IDLE, READ_ADDRESS, READ_DATA_FIRST_ROWS, READ_DATA_STRIDE, WRITE_ADDRESS, WRITE_ALL, WRITE_RESPONSE)
	...
	signal axi_wstrb    : std_loogic_vector(3 downto 0); 
	...
	begin 
		if(i_reset_n = '0') then 
			...
			when READ_ADDRESS => 
				...
				axi_arlen <= std_logic_vector(unsigned(input_volume_size)*unsigned(pool_kernel_size)); 
				axi_arvalid	<= '1';
			...
			when READ_ADDRESS_STRIDE => 
				axi_araddr <= input_data_addr_reg; 
				axi_arlen <= std_logic_vector(unsigned(input_volume_size)*unsigned(stride)); 
				axi_arvalid	<= '1';
			...
			when WRITE_ADDRESS => 
				...
				if(writes_remaining > 255 and calculated = '0') then 
					...
					axi_awlen <= std_logic_vector(unsigned(output_volume_size)*unsigned(output_volume_size)); 
					more_bursts_needed <= '0'; 
					writes_remaining <= (others => '0'); 
				else 
					axi_awlen <= axi_awlen; 
