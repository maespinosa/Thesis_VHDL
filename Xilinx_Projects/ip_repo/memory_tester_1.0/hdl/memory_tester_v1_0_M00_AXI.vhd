...
entity memory_tester_v1_0_M00_AXI is
	generic (
	...
	port (
		...
		ila_mt_m_axi_awaddr		: out std_logic_vector(C_M_AXI_ADDR_WIDTH-1 downto 0);
		...
		ila_mt_m_axi_wdata		: out std_logic_vector(C_M_AXI_DATA_WIDTH-1 downto 0);
		...
		ila_mt_m_axi_araddr		: out std_logic_vector(C_M_AXI_ADDR_WIDTH-1 downto 0);
		...
		ila_mt_m_axi_rdata 		: out std_logic_vector(C_M_AXI_DATA_WIDTH-1 downto 0);
...
end memory_tester_v1_0_M00_AXI;

architecture implementation of memory_tester_v1_0_M00_AXI is

	constant c_iterations : integer:= 524288-4-1; 
	constant c_iterations_sim : integer:= 10; 
	...
	constant c_base_address : std_logic_vector(31 downto 0) := x"8000_1000"; 
	-- AXI4FULL signals
	...
	signal iteration_counter : unsigned(31 downto 0); 
...
begin
	...
	ila_mt_rbc 				<= std_logic_vector(read_beat_counter); 
	ila_mt_data_counter 	<= std_logic_vector(data_counter); 
	ila_mt_address_counter  <= std_logic_vector(address_counter); 
	ila_mt_iteration_counter<= std_logic_vector(iteration_counter); 
	ila_mt_data_written		<= data_written;  
	ila_mt_error			<= error_detected; 
	ila_mt_data_read		<= data_read; 
		
	ila_mt_fsm_state  		<= fsm_state; 
	ila_mt_m_axi_awready	<= M_AXI_AWREADY;
	ila_mt_m_axi_awaddr		<= axi_awaddr; 
	ila_mt_m_axi_awsize   	<= axi_awsize; 
	ila_mt_m_axi_awlen    	<= axi_awlen; 
	ila_mt_m_axi_awvalid	<= axi_awvalid; 
	ila_mt_m_axi_wready 	<= M_AXI_WREADY; 
	ila_mt_m_axi_wdata		<= axi_wdata;
	ila_mt_m_axi_wlast		<= axi_wlast;
	ila_mt_m_axi_wvalid		<= axi_wvalid;
	ila_mt_m_axi_wstrb    	<= axi_wstrb;
	ila_mt_m_axi_bvalid 	<= M_AXI_BVALID;
	ila_mt_m_axi_bready		<= axi_bready;
	ila_mt_m_axi_wbc		<= std_logic_vector(write_beat_counter); 
	
	ila_mt_m_axi_arsize		<= axi_arsize;
	ila_mt_m_axi_araddr		<= axi_araddr; 
	ila_mt_m_axi_arlen    	<= axi_arlen;
	ila_mt_m_axi_arvalid	<= axi_arvalid;
	ila_mt_m_axi_rready		<= axi_rready;
	
	ila_mt_m_axi_arready 	<= M_AXI_ARREADY;
	ila_mt_m_axi_rdata 		<= M_AXI_RDATA;
	ila_mt_m_axi_rlast 		<= M_AXI_RLAST;
	ila_mt_m_axi_rvalid		<= M_AXI_RVALID;
	...
	next_state_comb: process(current_state,i_trigger,i_init_calib_complete,axi_wvalid, iteration_counter,axi_awlen,M_AXI_RVALID,M_AXI_ARREADY,M_AXI_RLAST,M_AXI_AWREADY,M_AXI_WREADY,M_AXI_BVALID, write_beat_counter) is 
	begin 
		axi_rready 	<= '0'; 
		axi_bready 	<= '0'; 
		...
		case current_state is 
		    when IDLE => 
				axi_rready <= '1'; 
				if (i_trigger = '1' and i_init_calib_complete = '1') then
            ...
            when WRITE_ADDRESS => 	
				axi_awaddr <= std_logic_vector(unsigned(c_base_address) + address_counter);
                axi_awsize <= "010";
                axi_awvalid <= '1'; 
				axi_awlen <= x"FF"; 
			...
			when WRITE_ALL => 
				axi_wdata <= std_logic_vector(data_counter); 
				axi_wstrb <= "1111";
				...
				if(M_AXI_WREADY = '1' and axi_wvalid = '1') then 					
					...
					if(write_beat_counter < unsigned(axi_awlen)-1) then 
						next_state <= WRITE_ALL;
						axi_wlast <= '0';
					else--if(write_beat_counter = unsigned(axi_awlen)-1) then
						--next_state <= WRITE_ALL;
						--axi_wlast <= '1';
						axi_wlast <= '1'; 
            ...
            when WRITE_RESPONSE => 
				...
				if(M_AXI_BVALID = '1') then 
					...
					axi_bready <= '1'; 
					if(iteration_counter < c_iterations-1 and i_debug_en = '0') then 
						next_state <= WRITE_ADDRESS; 
					elsif(iteration_counter < c_iterations_sim-1 and i_debug_en = '1') then 
						next_state <= WRITE_ADDRESS; 
					else 
						next_state <= CLEAR; 
					end if; 
			...
			when READ_ADDRESS => 
				axi_araddr <= std_logic_vector(unsigned(c_base_address) + address_counter);
				axi_arlen <= x"FF"; 
				axi_arvalid	<= '1';
				axi_arsize <= "010";  
			...
			when READ_DATA => 
				...
				if(M_AXI_RVALID = '1') then 
					...
					if(M_AXI_RLAST = '0') then 
						...
						if(data_counter > unsigned(M_AXI_RDATA) or data_counter < unsigned(M_AXI_RDATA)) then 
							error_detected <= '1'; 
						end if; 
