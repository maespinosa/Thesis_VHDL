...
begin
	...
	next_state_comb: process(current_state,i_control_reg,M_AXI_ARREADY,i_inbuff_full,M_AXI_RLAST,more_bursts_needed,i_softmax_complete,M_AXI_AWREADY,i_outbuff_empty,i_outbuff_valid,M_AXI_WREADY,write_beat_counter,M_AXI_BVALID) is 
	begin 
		axi_rready <= '0'; 
		axi_bready <= '0'; 
		outbuff_rd_en <= '0'; 
		...
		if(M_AXI_ARESETN = '0') then 
			...
			when IDLE => 
				if(i_slv_reg_wren(0) = '1') then 
					...
					reads_remaining <= unsigned(i_control_reg(31 downto 16)); 
				else 
			...
			when WRITE_ADDRESS => 
				...
				if(writes_remaining > 255 and calculated = '0') then 
					...
					axi_awlen <= std_logic_vector(to_unsigned(to_integer(unsigned(num_elements)),8)); 
					more_bursts_needed <= '0'; 
				else 
					axi_awlen <= axi_awlen; 
					more_bursts_needed <= more_bursts_needed; 
