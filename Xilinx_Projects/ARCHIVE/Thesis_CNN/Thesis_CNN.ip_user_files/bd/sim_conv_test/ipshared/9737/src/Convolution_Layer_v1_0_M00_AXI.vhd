...
begin
	...
	channels_allowed			<= i_conv_params_reg(23 downto 8); 
	...
	o_affine_params_reg			<= i_affine_params_reg; 
	channels_in_set 			<= i_affine_params_reg(15 downto 0); 
	filters_in_set			    <= i_affine_params_reg(31 downto 16); 
	...
	begin 
		...
		case current_state is 
		    when IDLE => 
				busy <= '0'; 
		    	if (start = '1' and i_layer_ready = '1' and i_volume_complete = '0') then 
			...
			when FCS_READ_DATA_WEIGHTS => 
				...
				if(i_inbuff_full = '0' and M_AXI_RLAST = '0') then 
					...
					if(M_AXI_RLAST = '1' and more_bursts_needed = '0') then 
						if(filter_counter < unsigned(number_of_filters)-1) then 
							next_state <= FCS_READ_ADDRESS_WEIGHTS;
						elsif(filter_counter >= unsigned(number_of_filters)-1) then 
			...
			when FCS_LOADING_WEIGHTS => 
				if(i_weights_loaded = '1' and iteration_counter = 0) then 
					next_state <= FCS_READ_ADDRESS_BIAS;
				elsif(i_weights_loaded = '1' and iteration_counter > 0) then -- and input_volume_row_counter < input_volume_height) then 
					next_state <= FCS_INPUT_VOLUME_SETUP; 
				-- elsif(i_weights_loaded = '1' and iteration_counter > 0 and input_volume_row_counter >= input_volume_height) then 
			...
			when FCS_WRITE_RESPONSE_CONV_OUT => 
				...
				if(M_AXI_BVALID = '1') then 
					...
					if(output_channel_counter < unsigned(output_volume_channels)-1 and iteration_counter = 0) then 
						next_state <= FCS_PROCESSING_DATA;
					elsif(output_channel_counter < unsigned(output_volume_channels)-1 and iteration_counter > 0) then 
						next_state <= RCS_PREV_DATA_SETUP; 
					else
						if(out_volume_row_counter < unsigned(output_volume_height)-1 and  input_volume_row_counter < unsigned(input_volume_height) and iteration_counter = 0) then 
							next_state <= FCS_STRIDE_PROCESSING; 
						elsif(out_volume_row_counter < unsigned(output_volume_height)-1 and  input_volume_row_counter >= unsigned(input_volume_height) and iteration_counter = 0) then 
							next_state <= FCS_PROCESSING_DATA; 
						elsif(out_volume_row_counter < unsigned(output_volume_height)-1 and iteration_counter > 0) then 
							next_state <= RCS_PREV_DATA_SETUP; 
						else 
			...
			when RCS_READ_DATA_PREV_DATA =>
				...
				if(i_prev_fifo_full = '0' and M_AXI_RLAST = '0') then 
					...
					if(M_AXI_RLAST = '1') then 
						if(i_weights_loaded = '0' and prev_channel_counter = 0) then 
							next_state <= FCS_READ_ADDRESS_WEIGHTS; 
						else 
							if(prev_channel_counter < unsigned(output_volume_channels)-1) then 
								next_state <= FCS_PROCESSING_DATA;
							elsif(prev_channel_counter >= unsigned(output_volume_channels)-1 and input_volume_row_counter < unsigned(input_volume_height)) then 
								next_state <= FCS_STRIDE_PROCESSING;
							elsif(prev_channel_counter >= unsigned(output_volume_channels)-1 and input_volume_row_counter >= unsigned(input_volume_height)) then 
								next_state <= FCS_PROCESSING_DATA; 
							else 
	...
	begin 
		if(M_AXI_ARESETN = '0') then 
			...
			when FCS_READ_DATA_WEIGHTS => 
				...
				if(i_inbuff_full = '0' and M_AXI_RLAST = '0') then 
					...
					if (M_AXI_RLAST = '1') then 
						...
						if(filter_counter < unsigned(number_of_filters)-1 and more_bursts_needed = '0') then 
							...
							reads_remaining <= unsigned(i_weight_multiple_0_reg)-1;  
						elsif(filter_counter >= unsigned(number_of_filters)-1 and more_bursts_needed = '0') then 
							...
							--weights_addr_counter <= weight_index + unsigned(i_weight_multiple_1_reg) + unsigned(i_weight_multiple_1_reg)+ unsigned(i_weight_multiple_1_reg)+ unsigned(i_weight_multiple_1_reg);  
							--weight_index <= weight_index + unsigned(i_weight_multiple_1_reg) + unsigned(i_weight_multiple_1_reg)+ unsigned(i_weight_multiple_1_reg)+ unsigned(i_weight_multiple_1_reg);  
						elsif(filter_counter < unsigned(number_of_filters)-1 and more_bursts_needed = '1') then 
							weights_addr_counter <= unsigned(weights_addr_counter) + g_bytes_per_data;
						else 
							weights_addr_counter <= unsigned(weights_addr_counter) + g_bytes_per_data;
						end if; 
				...
				if(i_inbuff_full = '0' and M_AXI_RLAST = '0') then 
					...
					if (M_AXI_RLAST = '1') then 
						...
						if(input_arsize = "001") then 
							if(channel_counter < unsigned(channels_allowed)-1 and more_bursts_needed = '0') then 
								...
								reads_remaining <= unsigned(i_input_multiple_1_reg)-1;  
							elsif(channel_counter >= unsigned(channels_allowed)-1 and more_bursts_needed = '0') then 
							...
							elsif(channel_counter < unsigned(channels_allowed)-1 and more_bursts_needed = '1') then 
			...
			when FCS_WRITE_RESPONSE_CONV_OUT =>  
				...
				if(M_AXI_BVALID = '1') then 
					data_written <= '1'; 
					if(output_channel_counter < unsigned(output_volume_channels)-1) then 
					...
					elsif(output_channel_counter >= unsigned(output_volume_channels)-1) then
						 
						output_channel_counter <= (others => '0'); 
						--output_addr_counter <= output_index + unsigned(output_volume_width) + unsigned(output_volume_width) + unsigned(output_volume_width) + unsigned(output_volume_width);
						--output_index <= output_index + unsigned(output_volume_width) + unsigned(output_volume_width) + unsigned(output_volume_width) + unsigned(output_volume_width);
						--output_base_pixel <= output_base_pixel + unsigned(output_volume_width) + unsigned(output_volume_width) + unsigned(output_volume_width) + unsigned(output_volume_width);
						...
						if(out_volume_row_counter < unsigned(output_volume_height)-1) then 
							...
							if(debug_mode = '0') then 
								...
								output_addr_counter <= unsigned(output_addr_counter) + g_bytes_per_data; 
								output_index <= unsigned(output_addr_counter) + g_bytes_per_data;
							end if; 
			...
			when RCS_READ_DATA_PREV_DATA => 
				...
				if(i_prev_fifo_full = '0' and M_AXI_RLAST = '0') then 
					...
					if (M_AXI_RLAST = '1') then 
						...
						elsif(prev_channel_counter >= unsigned(output_volume_channels)-1) then 
							prev_channel_counter <= (others => '0');--x"0001"; 
							--prev_index <= prev_addr_counter + 4 - out_last_channel_base; 
							--prev_addr_counter <= prev_addr_counter + 4 - out_last_channel_base; 
						-- else 
