...
entity OV5642_IF_v1_0_M00_AXI is
	generic (
...
end OV5642_IF_v1_0_M00_AXI;

architecture implementation of OV5642_IF_v1_0_M00_AXI is
	...
	-- Non-2^n lengths will eventually cause bursts across 4K address boundaries.
	 constant  C_MASTER_LENGTH  : std_logic_vector(11 downto 0) := x"4B0"; --x"00a"; --integer := 4; --600;
	-- total number of burst transfers is master length divided by burst length and burst size
	 constant  C_NO_BURSTS_REQ  : integer := 11; --(C_MASTER_LENGTH-clogb2((C_M_AXI_BURST_LEN*C_M_AXI_DATA_WIDTH/8)-1));
...
begin
    ...
    O_WRITE_NEXT <= wnext; 
    o_burst_write_active <= burst_write_active; 
    O_WREADY <= '1' when M_AXI_WREADY = '1' and write_index <= std_logic_vector(to_unsigned(255, C_TRANSACTIONS_NUM+1)) else 
                '0'; 
...
--	        elsif(i_outbuff_almost_empty = '0') then 
--	          axi_wvalid <= '1'; 
--	        elsif (wnext = '1' and axi_wlast = '1') then                                
--	          axi_wvalid <= '0';
--	        elsif (i_outbuff_almost_empty = '1') then 
--	          axi_wvalid <= '0';                                                         
--	        else                                                                        
	  ...
	  begin                                                                             
	    if (rising_edge (M_AXI_ACLK)) then                                              
	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                
	        ...
	        --if ((((write_index = std_logic_vector(to_unsigned(C_M_AXI_BURST_LEN-2,C_TRANSACTIONS_NUM+1))) and C_M_AXI_BURST_LEN >= 2)) or (C_M_AXI_BURST_LEN = 1)) then
              axi_wlast <= '1';                                                         
	    ...
	    if (rising_edge (M_AXI_ACLK)) then                                                                       
	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                                         
	        ...
	        if (M_AXI_AWREADY = '1' and axi_awvalid = '1') then                                                  
              ...
              if (write_burst_counter < C_MASTER_LENGTH )then                                   
	  ...
	  begin                                                                                                      
	    if (rising_edge (M_AXI_ACLK)) then                                                                       
	      ...
	      --elsif (M_AXI_RVALID && axi_rready && (read_burst_counter == {(C_NO_BURSTS_REQ-1){1}}) && axi_rlast)  
	        ...
	        if (M_AXI_BVALID = '1' and (write_burst_counter >= C_MASTER_LENGTH) and axi_bready = '1') then 
	          writes_done <= '1';                                                                                
	        end if;                                                                                              
	      end if;                                                                                                
	    end if;                                                                                                  
...
--          else   
--          axi_wdata <= axi_wdata;                                                 
--            if (wnext = '1'and i_outbuff_valid = '1') then                                                       
--              axi_wdata <= "0000000000000000" & DATA_TO_WRITE;                                             
--            end if;                                                                     
--          end if;                                                                       
--        end if;                                                                         
--      end process;
      
        process(DATA_TO_WRITE) 
        begin 
            axi_wdata <= "0000000000000000" & DATA_TO_WRITE; 
	...
	-- WVALID logic, similar to the axi_awvalid always block above                      
        process(i_outbuff_valid)                                                               
        begin                                                                             
                                                                        
             axi_wvalid <= i_outbuff_valid;                        
