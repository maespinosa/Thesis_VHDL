...
end VGA_IF_v1_0_M00_AXI;

architecture implementation of VGA_IF_v1_0_M00_AXI is
	...
	-- Non-2^n lengths will eventually cause bursts across 4K address boundaries.
	 constant  C_MASTER_LENGTH  : std_logic_vector(11 downto 0) := x"4B0"; --x"00a"; -- integer := 12;
...
begin
    ...
    O_READ_NEXT <= rnext; 
    o_burst_read_active <= burst_read_active; 
    O_RREADY <= axi_rready; 
    O_RVALID <= M_AXI_RVALID; 
    O_RDONE <= reads_done; 
    
    DATA_TO_READ <= M_AXI_RDATA(15 downto 0); 
...
--	  write_resp_error <= axi_bready and M_AXI_BVALID and M_AXI_BRESP(1);   
	...
	----------------------------------
	 ...
	 -- Forward movement occurs when the channel is valid and ready   
	  rnext <= M_AXI_RVALID and axi_rready; -- and not(i_inbuff_prog_full);                                 
	      ...
	      -- when M_AXI_RVALID is asserted by slave                         
	        ...
	        if (M_AXI_RVALID = '1') then                         
	          if (M_AXI_RLAST = '1' and axi_rready = '1') then   
	            axi_rready <= '0';                               
	          elsif(i_inbuff_prog_full = '1') then 
	            axi_rready <= '0'; 
	          else                                              
	             axi_rready <= '1';                              
	          end if;                                            
	    ...
	    if (rising_edge (M_AXI_ACLK)) then                                                                       
	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                                         
	        ...
	        if (M_AXI_ARREADY = '1' and axi_arvalid = '1') then                                                  
	          ...
	          if (read_burst_counter < C_MASTER_LENGTH) then                                                 
	  ...
	  begin                                                                                                      
	    if (rising_edge (M_AXI_ACLK)) then                                                                       
	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                                         
	        ...
	        if (M_AXI_RVALID = '1' and axi_rready = '1' and (read_index = std_logic_vector (to_unsigned(C_M_AXI_BURST_LEN-1,C_TRANSACTIONS_NUM+1))) and (read_burst_counter >= C_MASTER_LENGTH)) then
	             reads_done <= '1';                                                                                 
	        end if;                                                                                              
	      end if;                                                                                                
	    end if;                                                                                                  
