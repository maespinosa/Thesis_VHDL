...
architecture implementation of Max_Pool_Tester_v1_0_M00_AXI is
	...
	signal max_pool_axi_addr          : std_logic_vector(C_M_AXI_ADDR_WIDTH-1 downto 0) := x"8000_0000"; 
...
begin
	...
	next_state_comb: process(current_state,write_beat_counter, write_burst_counter, M_AXI_ARREADY,M_AXI_RLAST,M_AXI_AWREADY,M_AXI_WREADY,M_AXI_BVALID) is 
	begin 
		axi_rready <= '0'; 
		axi_bready <= '0'; 
		...
		case current_state is 
            ...
            when DATA_TO_MEM_WRITE_RESPONSE => 
				...
				if(M_AXI_BVALID = '1') then 
					if(write_burst_counter < 9075) then 
            ...
            when CONFIG_INPUT_DATA_ADDR_WRITE_ADDR => 
				if(M_AXI_AWREADY = '1') then 
                    next_state <= CONFIG_INPUT_DATA_ADDR_WRITE_DATA; --READ_ADDRESS; 
                else 
                    next_state <= CONFIG_INPUT_DATA_ADDR_WRITE_ADDR; --READ_DATA_FIRST_ROWS; 
                end if; 
            ...
            when CONFIG_OUTPUT_DATA_ADDR_WRITE_ADDR => 
				if(M_AXI_AWREADY = '1') then 
                    next_state <= CONFIG_OUTPUT_DATA_ADDR_WRITE_DATA; --READ_ADDRESS; 
                else 
                    next_state <= CONFIG_OUTPUT_DATA_ADDR_WRITE_ADDR; --READ_DATA_FIRST_ROWS; 
                end if; 
            ...
            when CONFIG_INPUT_PARAMS_WRITE_ADDR => 
				if(M_AXI_AWREADY = '1') then 
                    next_state <= CONFIG_INPUT_PARAMS_WRITE_DATA; --READ_ADDRESS; 
                else 
                    next_state <= CONFIG_INPUT_PARAMS_WRITE_ADDR; --READ_DATA_FIRST_ROWS; 
                end if; 
            ...
            when CONFIG_OUTPUT_PARAMS_WRITE_ADDR => 
				if(M_AXI_AWREADY = '1') then 
                    next_state <= CONFIG_OUTPUT_PARAMS_WRITE_DATA; --READ_ADDRESS; 
                else 
                    next_state <= CONFIG_OUTPUT_PARAMS_WRITE_ADDR; --READ_DATA_FIRST_ROWS; 
                end if; 
            ...
            when CONFIG_KERNEL_PARAMS_WRITE_ADDR => 
				if(M_AXI_AWREADY = '1') then 
                    next_state <= CONFIG_KERNEL_PARAMS_WRITE_DATA; --READ_ADDRESS; 
                else 
                    next_state <= CONFIG_KERNEL_PARAMS_WRITE_ADDR; --READ_DATA_FIRST_ROWS; 
                end if; 
            ...
            when CONFIG_CONTROL_WRITE_ADDR => 
				if(M_AXI_AWREADY = '1') then 
                    next_state <= CONFIG_CONTROL_WRITE_DATA; --READ_ADDRESS; 
                else 
                    next_state <= CONFIG_CONTROL_WRITE_ADDR; --READ_DATA_FIRST_ROWS; 
                end if; 
		...
		if(M_AXI_ARESETN = '0') then 
			...
			case current_state is 
                ...
                when IDLE => 
                    ...
                    ddr_mem_addr <= x"8000_0000"; 
                    
                when DATA_TO_MEM_WRITE_ADDRESS => 
                    axi_awaddr <= ddr_mem_addr;
                    axi_awlen <= x"1F"; 
                    axi_awsize <= "001";
                    axi_awvalid <= '1'; 
                    axi_wstrb <= "1100"; 
                ...
                when DATA_TO_MEM_WRITE_DATA => 
                    ...
                    if(M_AXI_WREADY = '1') then 
                        ...
                        ddr_mem_addr <= std_logic_vector(unsigned(ddr_mem_addr) + 4); 
                    else 
                        axi_wdata <= (others => '0'); 
                        axi_wvalid <= '0'; 
                        axi_wstrb <= axi_wstrb; 
   ...
   --             when READ_OUTPUT_READ_DATA => 
      ...
      begin 
        if (M_AXI_ARESETN = '0' or close_flag = '1') then 
